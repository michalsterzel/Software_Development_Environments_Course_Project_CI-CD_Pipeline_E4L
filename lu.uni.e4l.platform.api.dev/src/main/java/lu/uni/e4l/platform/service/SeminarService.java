...
public class SeminarService {
    ...
    public List<Seminar> getSeminarList() {
        List<SeminarView> temp = StreamSupport.stream(seminarRepository.findBy().spliterator(), true)
                .sorted(Comparator.comparing(SeminarView::getEventDateTime).reversed())
                .collect(Collectors.toList());
        List<Seminar> seminars = new ArrayList<Seminar>();
        for (SeminarView seminarView : temp) {
            seminars.add(Seminar.fromSeminarView(seminarView));
        }
        return seminars;
    }

    public long getSeminarListOne(long id) {
        return getSeminar(id).getSeminarCounter();
    }
    ...
    public SeminarView getSeminarView(long id) {
        return StreamSupport.stream(seminarRepository.findBy().spliterator(), true)
                .sorted(Comparator.comparing(SeminarView::getEventDateTime).reversed())
                .filter(seminar -> seminar.getId() == id).findFirst().orElse(null);
    }
    ...
    public Seminar updateSeminarStatus(Seminar newSeminar) {
        ...
        if (previousStatus.equals(newSeminar.getStatus())) {
            canUpdate = true;
        } else if (previousStatus.equals(SeminarStatus.TODO) && (newSeminar.getStatus().equals(SeminarStatus.OPEN)
                || newSeminar.getStatus().equals(SeminarStatus.CANCELLED))) {
            canUpdate = true;
        } else if (previousStatus.equals(SeminarStatus.OPEN) && (newSeminar.getStatus().equals(SeminarStatus.TODO)
                || newSeminar.getStatus().equals(SeminarStatus.CLOSED))) {
            canUpdate = true;
        }
        if (canUpdate) {
            seminar.setStatus(newSeminar.getStatus());
            seminar = seminarRepository.save(seminar);
            return Seminar.fromSeminarView(getSeminarView(seminar.getId()));
        } else {
            throw new InvalidSeminarException(
                    "The current seminar status does not allow to change to the provided status");
        }
    }
